...
def insert_loans(conn, data):
    cur = conn.cursor()
    rows = [
        (row['loan_id'], row['client_id'], row['loan_amount'], row['interest_rate'], row['loan_date'])
        for row in data
    ]
    ...
    execute_values(
        cur,
        f"INSERT INTO {TABLE_NAME} (loan_id, client_id, loan_amount, interest_rate, loan_date) VALUES %s",
        rows
    )
    conn.commit()
